...
combinations = []
item = [A,B,CC]
combinations.append(copy.deepcopy(item))
item = [A,B,D]
combinations.append(copy.deepcopy(item))
item = [A,B,E]
combinations.append(copy.deepcopy(item))
...
if __name__ == '__main__':
    ...
    # 读入飞机位置数据
    messages = pd.read_csv("MessagesReceivedByRadarcapeBy3_11w_322_394_436_13_115_MonitorArea0.csv")
    # print(messages.info())
    ...
    # 记录每一种组合对应的预测的top5 10种组合  共top50
    results = []
    count = 0
    for index,row in messages.iterrows():
        ...
        # 这里需要注意 这里就需要对 消息中的传感器编号做组合了
        list = [0,0,0]
        count = 1
        for combination in combinations:
            # 根据当前的组合 拼接字符串 名字为网格组合的名字
            name = "Grid_600m_combination_" + str(count) + ".npy"
            print("正在读取组合" + str(count) + "对应的网格数据")
            grid = np.load(name)
            grid_numpy = grid[:, 3:6]
            # print(grid_numpy.shape)
            print("正在计算消息数据的组合" + str(count) + "对应的tdoa")
            for i in range(len(j)):
                if sensors.__contains__(j[i][0]):
                    if j[i][0] == combination[0]:
                        list[0] = j[i][1]
                    if j[i][0] == combination[1]:
                        list[1] = j[i][1]
                    if j[i][0] == combination[2]:
                        list[2] = j[i][1]

            # 根据当前的传感器的组合数据 计算tdoa
            tdoa = calcTDOAByMessage(list)
            # 拿到tdoa后 可以开始预测位置了
            # 这里返回的是topk的索引 然后根据索引再去grid表中找对应的位置信息 再求平均值
            top_k_index = calcTopKInex(tdoa,grid_numpy)
            print("完成组合" + str(count) + "的预测，保存当前组合的top5数据")
            count+=1
            results.append(copy.deepcopy(top_k_index))
            # print("******************************************")
            ...
            print("当前组合" + str(count) + "的预测误差为:", error)
            # print(error)
            # # 保存数据
        ...
        print("完成了当前消息的所有组合的预测，对top50的数据再进行一次分析")
        count = 1
        grid = np.load("Grid_600m_s.npy")
        sum_lat = 0
        sum_lon = 0
        for top5 in results:
            print("对组合"+str(count) + "的预测情况进行分析")
            print("组合"+str(count) + "top5个点到飞机的真实位置的距离依此为")
            for pos in top5:
                sum_lat += grid[pos][1]
                sum_lon += grid[pos][2]
                error = geodesic((lat, lon), ( grid[pos][1] ,  grid[pos][2] )).m
                print(str(error) + "m")
            count+=1
        # sum_lat = sum_lat/50
        # sum_lon = sum_lon/50
        # error = geodesic((lat, lon), (sum_lat,sum_lon)).m
        # print("top50个网格的平均位置为",sum_lat,sum_lon,"和飞机的真实位置误差为:",error)
