...
# 50m在纬度上的距离
deleta_lon = (4.142814 - 4.142087166666667)
deleta_lat = (51.833122 - 51.83267283333333)

if __name__ == '__main__':

    print("生成基于%d个sensors的%dm大小的网格"%(len(sensors),SQURE_SIZE))
    ...
    grid_size = int(LENGTH/SQURE_SIZE) * int(WIDTH/SQURE_SIZE)
    grid = np.zeros((grid_size, 6), dtype=object)
    ...
    for i in range(int(LENGTH/SQURE_SIZE)):
        ...
        for j in range(int(WIDTH/SQURE_SIZE)):
            # 每一行纬度不变 经度递增
             ...
             # 将上述信息组合为一个numpy
             temp = np.array([ecef[0],ecef[1],ecef[2],tdoa[0],tdoa[1],tdoa[2]])
             # 这里注意为了避免最后的numpy数组空一行（即第一行）,这里需要判断一下
             # grid = np.vstack([grid,temp])
             # 注意这里要用深拷贝
             grid[id] = np.copy(temp)
             id+=1
             if id%10000 == 0:
    ...
    np.save("Grid_50m_s_3s_ecef.npy",grid)
