...
# @Abstract: 位置预测算法 Demo1 用很少量的数据集来测试
...
def calcLocationEstimation(top_k_index,grid):
    ...
    for i in range(len(top_k_index)):
        sum_lat += grid.loc[top_k_index[i],'latitude']
        sum_lon += grid.loc[top_k_index[i],'longitude']
...
if __name__ == '__main__':
    # 先读取网格数据
    grid = pd.read_csv("Grid_50m_test.csv")
    # print(grid.info())
    ...
    # 保存数据
    predictError.to_csv("predictError_50m_test.csv")
