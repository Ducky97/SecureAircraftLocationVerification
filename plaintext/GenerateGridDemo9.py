...
# 监控区域到地面的距离 11km
HEIGHT = Decimal(11582)
# 简单的网格计算 500m * 500m
WIDTH = Decimal(150 *1000)
LENGTH = Decimal(220 * 1000)
SQURE_SIZE = Decimal(600)
# 光速 m/s
C = Decimal(299792458)

# 三个sensor的信息 也初始化为常量
sensor_A_lat = Decimal(51.833122)
sensor_A_lon = Decimal(4.142814)
sensor_A_height = Decimal(-1.820427)

sensor_B_lat = Decimal(51.242813)
sensor_B_lon = Decimal(6.684991)
sensor_B_height = Decimal(43.901066)

sensor_C_lat = Decimal(51.341648)
sensor_C_lon = Decimal(5.893123)
sensor_C_height = Decimal(39.085175)
...
def calcDistanceContainHeight(point_distance,sensor_height):
    ...
    :return: 
    '''
    height = Decimal(HEIGHT - sensor_height)
    sum = Decimal(point_distance**2 + height**2)
...
def calcTime(lat,lon,sensor_lat,sensor_lon,sensor_height):
    ...
    '''
    # 先计算两点的距离
    distance = Decimal(geodesic((lat, lon), ( sensor_lat ,  sensor_lon )).m)
    # 再计算加上高度的距离
    distance = Decimal(calcDistanceContainHeight(distance,sensor_height))
    # 把km->m
    # 单位 秒
    time = Decimal(distance / C)
    # 换成 纳秒
    return time
...
# 600m在纬度上的距离
deleta_lon = Decimal((4.142814-4.13409))
deleta_lat = Decimal((51.833122-51.82773))

if __name__ == '__main__':
    # lat纬度 lon 经度
    lat_A =  Decimal(51.833122)
    lon_A = Decimal(4.142814)
    ...
    # 保存
    # ns代表tdoa精度为 s秒
    np.save("Grid_600m_s_demical.npy",grid)
