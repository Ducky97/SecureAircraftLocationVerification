...
# 修改tdoa算法后的测试类
# 基于ecef坐标系的预测算法
# 采用3个sensor 322,436,13,394 然后基于300m网格的测试
...
def calcLocationEstimation(top_k_index,grid):
    ...
    for i in range(len(top_k_index)):
        sum_X += grid[top_k_index[i]][0]
        sum_Y += grid[top_k_index[i]][1]
        sum_Z += grid[top_k_index[i]][2]
...
if __name__ == '__main__':
    ...
    print("基于%d个sensors的%dm大小的网格进行预测" % (len(sensors), SQURE_SIZE))
    # 先读取网格数据
    grid = np.load("Grid_50m_s_3s_ecef.npy")
    ...
    # messages = pd.read_csv("Messages_150m_500.csv")
    # print(messages.info())
    ...
    print("完成%d条消息的预测,预测基于%d个传感器生成的%dm网格" % (count,len(sensors),SQURE_SIZE))
    ...
    print("开始保存预测数据")
    predictError.to_csv("predictError_Grid_50m_s_3sensor_ecef.csv",index=None)
