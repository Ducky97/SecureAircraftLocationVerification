...
# @Author  : QixianZhou
# @FileName: EncLocEstimationDemo02.py
...
if __name__ == '__main__':
    ...
    # 读取飞机消息数据
    messages = pd.read_csv("Messages_300m_500.csv")

    # 读取网格的密文tdoa 和相关数据
    EncTDOA = np.load("EncTDOA_300m.npy")
    EncTDOA_M = np.load("EncTDOA_300m_M.npy")
    EncTDOA_H = np.load("EncTDOA_300m_H.npy")
    EncTDOA_S = np.load("EncTDOA_300m_S.npy")

    # 读取网格的密文位置和密钥
    EncCOOR = np.load("EncCOOR_300m.npy")
    EncCOOR_S = np.load("EncCOOR_300m_S.npy")
    ...
    print("开始保存预测数据")
    predictError.to_csv("EncPredictError_Grid_300m_s_3sensor_ecef_500message.csv", index=None)
