...
# @Author  : QixianZhou
# @FileName: EncQuicklyIndetifyDemo01.py
# @Abstract: 快速定位算法 Demo01
...
def quicklyIdentify(plane_state_lat,plane_state_lon,plane_tdoa,grid_real):
    '''
...
def encQuicklyIdentify(plane_state_lat,plane_state_lon,predict_lat,predict_lon):
    error_llh = geodesic((plane_state_lat, plane_state_lon), (predict_lat, predict_lon)).m
...
if __name__ == '__main__':
    ...
    for index, row in messages.iterrows():
        ...
        NUMBER = grid_coor.shape[0]
        DIMIENSION = grid_coor.shape[1]
        # print("将原始的coor数据转换为int类型,放大系数为：%d" % (S2NS))
        coor_int = np.zeros((NUMBER, DIMIENSION), dtype=object)
        for i in range(NUMBER):
            coor_int[i] = float2Int(grid_coor[i])

        T = vhe.getRandomMatrix(DIMIENSION, 1, vhe.tBound)
        S_COOR = vhe.getSecretKey(T)
        I = np.eye(DIMIENSION, dtype=object)
    ...
    print("完成密文下%d条飞机消息合法性的快速认证"%(len(messages)))
    print("TP=",TP)
    print("FP=",FP)
    print("FN=", FN)
    print("TN=", TN)
    ...
    print("密文下，在阈值=%d的条件下，各项检测指标为"%THRESHOLD)
